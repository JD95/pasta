...
statsReporter = TestReporter optDesc runner
 where
  optDesc = [Option (Proxy :: Proxy (Maybe StatsFile))]
  runner opts tree = do
    StatsFile file <- lookupOption opts
    pure
      $ collectStats (getNumThreads $ lookupOption opts) file
      $ IntMap.fromList
      $ zip [0 ..]
...
collectStats
  :: Int -> FilePath -> IntMap TestName -> StatusMap -> IO (Time -> IO Bool)
...
git args = readProcessWithExitCode "git" args "" >>= pure . \case
  (ExitSuccess  , out, _) -> dropWhileEnd isSpace out
...
resultRow nthreads' results = do
  ...
  pure
    $ flip map results
    $ \(show -> idx, (name, Result { resultDescription = dropWhileEnd isSpace -> description, resultShortDescription = result, resultTime = show -> time })) ->
        [ idx
        , name
        , time
        , result
