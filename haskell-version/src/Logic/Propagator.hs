...
adder in1 in2 out = do
  waitOn [SomeCell in1, SomeCell in2] $ propagate (add in1 in2) [out]
  waitOn [SomeCell out, SomeCell in2] $ propagate (sub out in2) [in1]
  waitOn [SomeCell out, SomeCell in1] $ propagate (sub out in1) [in2]

product :: (Merge a, Floating a, MonadFail m, Network m, Cell m f, Cell m g, Cell m h) => f a -> g a -> h a -> m ()
product in1 in2 out = do
  waitOn [SomeCell in1, SomeCell in2] $ propagate (liftBinOp (*) in1 in2) [out]
  waitOn [SomeCell out, SomeCell in2] $ propagate (liftBinOp (/) out in2) [in1]
  waitOn [SomeCell out, SomeCell in1] $ propagate (liftBinOp (/) out in1) [in2]
